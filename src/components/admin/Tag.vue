<template>
  <div>
    ...
    >
      <el-form-item prop="content" label="添加一个标签">
        <el-input v-model="tagFormRules.content" placeholder="输入一个标签"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="addTag">添加</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="tagList" :stripe="true" :border="true" style="width: 100%" height="430px">
      ...
      <!-- <el-table-column align="center" width="160" fixed="right" prop="content" label="操作"> -->
      <el-table-column align="center" width="160"  prop="content" label="操作">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    async init() {
      const { data: tagList, status } = await this.$http.get('/tag/admin')
      if (status === 200) {
    ...
    async deleteRow(scope) {
      const { status } = await this.$http.delete('/tag/admin/' + scope.row.id)
      if (status === 200) {
        this.$message('移除成功')
        this.tagList.splice(scope.$index, 1)
      }
    },
    ...
    async updateSubmit() {
      this.loading = true
      const { status } = await this.$http.post('/tag/admin', this.updateForm)
      if (status === 200) {
    ...
    async addTag() {
      const { status } = await this.$http.put('/tag/add', {
        content: this.tagFormRules.content
      })
      if (status === 200) {
